...
jQuery(document).ready(function($) {
    ...
    $("#prev").click(function() {
        var select=$("[name=wx_tSelect]");
        var index=select[0].selectedIndex;
        select[0].selectedIndex=index-1;
        updateTime(select.val(), false);
    });
    
    $("#next").click(function() {
        var select=$("[name=wx_tSelect]");
        updateTime(select.val(), true);
        var index=select[0].selectedIndex;
        select[0].selectedIndex=index+1;
    });
...
function make_splitdemo() {
    ...
    document.getElementById('tSelect').appendChild(wx1.tselect);
    wx1.linkTime(wx2);
    wx1.linkTime(wx3);
    wx1.linkTime(wx4);
}
